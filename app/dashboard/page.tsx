...
const GRANDFATHERED_CUTOFF = '2026-05-10'
...
function ReviewCard({ review }: { review: any }) {
  ...
  return (
    <div className="bg-white/10 rounded-lg p-3">
      ...
      <p className="text-blue-200 text-xs">{expanded || !isLong ? review.text : review.text?.substring(0, 200) + '...'}</p>
      {isLong && (
        <button onClick={() => setExpanded(!expanded)} className="text-blue-400 text-xs mt-1 hover:text-white transition">
...
function UpgradePrompt({ feature }: { feature: string }) {
  return (
    <div className="bg-white/5 rounded-xl p-6 text-center border border-white/10">
      <p className="text-white font-medium mb-2">🔒 {feature}</p>
      <p className="text-blue-300 text-sm mb-4">This feature is available on the Growth plan and above.</p>
      <Link href="/pricing" className="bg-white text-blue-900 px-4 py-2 rounded-lg font-semibold text-sm hover:bg-blue-50 transition">
...
export default function Dashboard() {
  ...
  // Check if user has access to Growth features
  const hasGrowthAccess = isGrandfathered ||
    subscription?.plan === 'growth' ||
    subscription?.plan === 'pro' ||
    subscription?.status === 'trialing'

  const hasProAccess = isGrandfathered ||
    subscription?.plan === 'pro' ||
    subscription?.status === 'trialing'
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-blue-950 via-blue-900 to-indigo-900 p-3 sm:p-6">
      <div className="max-w-5xl mx-auto px-2 sm:px-0">
        ...
        {/* No subscription banner for new users */}
        {!isGrandfathered && !subscription && (
          <div className="bg-blue-500/20 border border-blue-500/30 rounded-xl p-4 mb-6 flex justify-between items-center">
            <div>
              <p className="text-blue-200 font-medium text-sm">🚀 Start your 14-day free trial</p>
              <p className="text-blue-300 text-xs mt-1">Choose a plan to unlock all ShopCast features.</p>
            </div>
            <Link href="/pricing" className="bg-white text-blue-900 px-4 py-2 rounded-lg font-semibold text-sm hover:bg-blue-50 transition">
              View Plans
            </Link>
        ...
        {!isGrandfathered && subscription?.status === 'canceled' && (
          <div className="bg-red-500/20 border border-red-500/30 rounded-xl p-4 mb-6 flex justify-between items-center">
            <div>
              <p className="text-red-300 font-medium text-sm">⚠️ Your subscription has ended</p>
              <p className="text-red-200 text-xs mt-1">Resubscribe to continue using ShopCast.</p>
            </div>
            <Link href="/pricing" className="bg-white text-red-900 px-4 py-2 rounded-lg font-semibold text-sm hover:bg-red-50 transition">
              Resubscribe
            </Link>
          </div>
        )}
        ...
        <div className="bg-white/10 rounded-2xl p-6 mb-6">
          ...
          {!hasGrowthAccess ? <UpgradePrompt feature="Marketing Recommendations" /> : marketingRecs ? (
            <>
              ...
              {marketingRecs.quickWins?.length > 0 && (
                <div className="bg-white/10 rounded-xl p-4 mb-4">
                  ...
                  <ul className="flex flex-col gap-1">{marketingRecs.quickWins.map((win: string, i: number) => <li key={i} className="text-blue-100 text-sm">• {win}</li>)}</ul>
                </div>
              )}
              {marketingRecs.weeklyBudgetBreakdown && (
                <div className="bg-white/10 rounded-xl p-4 mb-4">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                    <div className="bg-white/10 rounded-lg p-3 text-center"><p className="text-white font-bold">${marketingRecs.weeklyBudgetBreakdown.googleAds}</p><p className="text-blue-300 text-xs">Google Ads</p></div>
                    <div className="bg-white/10 rounded-lg p-3 text-center"><p className="text-white font-bold">${marketingRecs.weeklyBudgetBreakdown.metaAds}</p><p className="text-blue-300 text-xs">Meta Ads</p></div>
                    <div className="bg-white/10 rounded-lg p-3 text-center"><p className="text-white font-bold">${marketingRecs.weeklyBudgetBreakdown.promotions}</p><p className="text-blue-300 text-xs">Promotions</p></div>
                    <div className="bg-white/10 rounded-lg p-3 text-center"><p className="text-green-400 font-bold">${marketingRecs.weeklyBudgetBreakdown.total}</p><p className="text-blue-300 text-xs">Total</p></div>
                  </div>
                </div>
              )}
              {marketingRecs.googleAds && (
                <div className="bg-white/10 rounded-xl p-4 mb-4">
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-3">
                    <div><p className="text-blue-300 text-xs">Budget</p><p className="text-white font-bold">${marketingRecs.googleAds.recommendedBudget}/day</p></div>
                    <div><p className="text-blue-300 text-xs">Est. CPC</p><p className="text-white font-bold">{marketingRecs.googleAds.estimatedCPC}</p></div>
                  </div>
                  <div className="mb-3">
                    <p className="text-blue-300 text-xs mb-1">Top Keywords</p>
                    <div className="flex flex-wrap gap-1">{marketingRecs.googleAds.topKeywords?.map((kw: string, i: number) => <span key={i} className="bg-white/10 text-blue-200 px-2 py-1 rounded text-xs">{kw}</span>)}</div>
                  </div>
                  <div className="bg-white/10 rounded-lg p-3"><p className="text-blue-300 text-xs mb-1">💡 Ad Copy</p><p className="text-white text-sm font-medium">"{marketingRecs.googleAds.adCopyIdea}"</p></div>
                </div>
              )}
              {marketingRecs.metaAds && (
                <div className="bg-white/10 rounded-xl p-4 mb-4">
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-3">
                    <div><p className="text-blue-300 text-xs">Budget</p><p className="text-white font-bold">${marketingRecs.metaAds.recommendedBudget}/day</p></div>
                    <div><p className="text-blue-300 text-xs">Format</p><p className="text-white font-bold capitalize">{marketingRecs.metaAds.bestFormat}</p></div>
                  </div>
                  <div className="mb-3"><p className="text-blue-300 text-xs mb-1">Target Audience</p><p className="text-blue-100 text-sm">{marketingRecs.metaAds.targetAudience}</p></div>
                  <div className="bg-white/10 rounded-lg p-3"><p className="text-blue-300 text-xs mb-1">💡 Ad Idea</p><p className="text-white text-sm font-medium">{marketingRecs.metaAds.adCopyIdea}</p></div>
                </div>
              )}
              {marketingRecs.promotions?.length > 0 && (
                <div className="bg-white/10 rounded-xl p-4 mb-4">
                  <p className="text-blue-300 text-xs font-medium mb-3">🏷️ Recommended Promotions</p>
                  <div className="flex flex-col gap-3">{marketingRecs.promotions.map((promo: any, i: number) => (
                    <div key={i} className="bg-white/10 rounded-lg p-3">
                      <div className="flex justify-between items-start mb-1"><p className="text-white font-medium text-sm">{promo.name}</p><span className="text-green-400 text-xs font-bold ml-2">{promo.expectedLift}</span></div>
              ...
              {marketingRecs.contentCalendar?.length > 0 && (
                <div className="bg-white/10 rounded-xl p-4">
                  ...
                  <div className="flex flex-col gap-2">{marketingRecs.contentCalendar.map((item: any, i: number) => (
                    <div key={i} className="flex gap-3 items-start bg-white/10 rounded-lg p-3">
                      <div className="flex-shrink-0"><p className="text-white text-xs font-bold">{item.day}</p><p className="text-blue-300 text-xs">{item.platform}</p></div>
        ...
        {(googleAdsConnected || metaAdsConnected) && (
          <div className="bg-white/10 rounded-2xl p-6 mb-6">
            ...
            {!hasGrowthAccess ? <UpgradePrompt feature="Ad Budget Optimization" /> : adOptimization ? (
              <>
              ...
              <p className="text-blue-300 text-sm">Click "Optimize" to get AI-powered ad budget recommendations.</p>
            )}
          </div>
        )}
        ...
        <div className="bg-white/10 rounded-2xl p-6 mb-6">
          ...
          {!hasGrowthAccess ? <UpgradePrompt feature="Google Reviews Monitoring" /> : reviews ? (
            <>
              <div className={`rounded-xl p-4 mb-4 flex items-center gap-4 ${reviews.alertLevel === 'critical' ? 'bg-red-500/20' : reviews.alertLevel === 'warning' ? 'bg-yellow-500/20' : 'bg-green-500/20'}`}>
                <div className="text-center">
                  ...
                  <div className="flex gap-0.5 mt-1">{[1,2,3,4,5].map(star => <span key={star} className={`text-lg ${star <= Math.round(reviews.googleRating) ? 'text-yellow-400' : 'text-white/30'}`}>★</span>)}</div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                {reviews.commonPraise?.length > 0 && <div className="bg-white/10 rounded-xl p-4"><p className="text-green-400 text-xs font-medium mb-2">👍 What Customers Love</p><ul className="flex flex-col gap-1">{reviews.commonPraise.map((p: string, i: number) => <li key={i} className="text-blue-100 text-xs">• {p}</li>)}</ul></div>}
                {reviews.commonComplaints?.length > 0 && <div className="bg-white/10 rounded-xl p-4"><p className="text-red-400 text-xs font-medium mb-2">👎 Common Complaints</p><ul className="flex flex-col gap-1">{reviews.commonComplaints.map((c: string, i: number) => <li key={i} className="text-blue-100 text-xs">• {c}</li>)}</ul></div>}
              </div>
              {reviews.areasToImprove?.length > 0 && <div className="bg-white/10 rounded-xl p-4 mb-4"><p className="text-yellow-400 text-xs font-medium mb-2">🔧 Areas to Improve</p><ul className="flex flex-col gap-1">{reviews.areasToImprove.map((a: string, i: number) => <li key={i} className="text-blue-100 text-xs">• {a}</li>)}</ul></div>}
              {reviews.customerSuggestions?.length > 0 && <div className="bg-white/10 rounded-xl p-4 mb-4"><p className="text-blue-300 text-xs font-medium mb-2">💡 What Customers Are Asking For</p><ul className="flex flex-col gap-1">{reviews.customerSuggestions.map((s: string, i: number) => <li key={i} className="text-blue-100 text-xs">• {s}</li>)}</ul></div>}
              {reviews.recentNegative?.length > 0 && <div className="bg-red-500/10 rounded-xl p-4 mb-4"><p className="text-red-400 text-xs font-medium mb-2">🚨 Recent Negative Reviews</p><div className="flex flex-col gap-2">{reviews.recentNegative.map((r: any, i: number) => <ReviewCard key={i} review={r} />)}</div></div>}
              {reviews.recentPositive?.length > 0 && <div className="bg-green-500/10 rounded-xl p-4 mb-4"><p className="text-green-400 text-xs font-medium mb-2">🌟 Recent Positive Reviews</p><div className="flex flex-col gap-2">{reviews.recentPositive.map((r: any, i: number) => <ReviewCard key={i} review={r} />)}</div></div>}
              {reviews.googleMapsUrl && <a href={reviews.googleMapsUrl} target="_blank" rel="noopener noreferrer" className="block text-center text-blue-300 text-xs hover:text-white transition mt-2">View all reviews on Google Maps →</a>}
            </>
          ) : (
            <p className="text-blue-300 text-sm">Click "Load Reviews" to see your Google rating and recent customer feedback.</p>
          )}
        </div>
        ...
        {competitors.length > 0 && (
          <div className="bg-white/10 rounded-2xl p-6 mb-6">
            ...
            {!hasGrowthAccess ? <UpgradePrompt feature="Competitor Analysis" /> : competitorAnalysis ? (
              <>
                <div className="flex flex-col gap-2 mb-4">
                  ...
                  <div className="bg-white/10 rounded-xl p-3 flex justify-between items-center">
                    ...
                    <span className={`text-xs font-bold px-2 py-1 rounded-full ${competitorAnalysis.myTraffic?.trafficColor === 'green' ? 'bg-green-500 text-white' : competitorAnalysis.myTraffic?.trafficColor === 'yellow' ? 'bg-yellow-500 text-white' : 'bg-red-500 text-white'}`}>{competitorAnalysis.myTraffic?.trafficLabel || 'Unknown'}</span>
                  </div>
                  {competitorAnalysis.competitorTraffic?.map((comp: any, i: number) => (
                    <div key={i} className="bg-white/10 rounded-xl p-3 flex justify-between items-center">
                      ...
                      <span className={`text-xs font-bold px-2 py-1 rounded-full ${comp.traffic?.trafficColor === 'green' ? 'bg-green-500 text-white' : comp.traffic?.trafficColor === 'yellow' ? 'bg-yellow-500 text-white' : comp.traffic?.trafficColor === 'red' ? 'bg-red-500 text-white' : 'bg-gray-500 text-white'}`}>{comp.traffic?.trafficLabel || 'No data'}</span>
                    </div>
                  ))}
                </div>
                ...
                {competitorAnalysis.opportunities?.length > 0 && <div className="bg-white/10 rounded-xl p-4 mb-4"><p className="text-blue-300 text-xs font-medium mb-2">✅ Opportunities</p><ul className="flex flex-col gap-1">{competitorAnalysis.opportunities.map((o: string, i: number) => <li key={i} className="text-blue-100 text-sm">• {o}</li>)}</ul></div>}
                {competitorAnalysis.threats?.length > 0 && <div className="bg-white/10 rounded-xl p-4 mb-4"><p className="text-blue-300 text-xs font-medium mb-2">⚠️ Threats</p><ul className="flex flex-col gap-1">{competitorAnalysis.threats.map((t: string, i: number) => <li key={i} className="text-blue-100 text-sm">• {t}</li>)}</ul></div>}
              ...
              <p className="text-blue-300 text-sm">Click "Analyze" to compare traffic with competitors.</p>
            )}
          </div>
        )}
        ...
        {store?.website_url ? (
          <div className="bg-white/10 rounded-2xl p-6 mb-6">
            ...
            {!hasGrowthAccess ? <UpgradePrompt feature="Website & Social Scanner" /> : websiteScan ? (
              <>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
                  <div className="bg-white/10 rounded-xl p-3 text-center"><p className="text-blue-300 text-xs mb-1">Online Presence</p><p className={`font-bold text-sm ${websiteScan.onlinePresence === 'strong' ? 'text-green-400' : websiteScan.onlinePresence === 'moderate' ? 'text-yellow-400' : 'text-red-400'}`}>{websiteScan.onlinePresence?.charAt(0).toUpperCase() + websiteScan.onlinePresence?.slice(1)}</p></div>
                  <div className="bg-white/10 rounded-xl p-3 text-center"><p className="text-blue-300 text-xs mb-1">Sentiment</p><p className={`font-bold text-sm ${websiteScan.socialSentiment === 'positive' ? 'text-green-400' : websiteScan.socialSentiment === 'negative' ? 'text-red-400' : 'text-yellow-400'}`}>{websiteScan.socialSentiment?.charAt(0).toUpperCase() + websiteScan.socialSentiment?.slice(1)}</p></div>
                  <div className="bg-white/10 rounded-xl p-3 text-center"><p className="text-blue-300 text-xs mb-1">Web Traffic</p><p className={`font-bold text-sm ${websiteScan.trafficEstimate === 'high' ? 'text-green-400' : websiteScan.trafficEstimate === 'medium' ? 'text-yellow-400' : 'text-red-400'}`}>{websiteScan.trafficEstimate?.charAt(0).toUpperCase() + websiteScan.trafficEstimate?.slice(1)}</p></div>
                  <div className="bg-white/10 rounded-xl p-3 text-center"><p className="text-blue-300 text-xs mb-1">Price Range</p><p className="text-white font-bold text-sm">{websiteScan.priceRange?.charAt(0).toUpperCase() + websiteScan.priceRange?.slice(1)}</p></div>
                </div>
                <div className="bg-white/10 rounded-xl p-4 mb-4"><p className="text-blue-300 text-xs font-medium mb-2">📊 Summary</p><p className="text-blue-100 text-sm">{websiteScan.summary}</p></div>
                {websiteScan.promotions?.length > 0 && <div className="bg-white/10 rounded-xl p-4 mb-4"><p className="text-blue-300 text-xs font-medium mb-2">🏷️ Current Promotions Found</p><ul className="flex flex-col gap-1">{websiteScan.promotions.map((p: string, i: number) => <li key={i} className="text-blue-100 text-sm">• {p}</li>)}</ul></div>}
                {websiteScan.opportunities?.length > 0 && <div className="bg-white/10 rounded-xl p-4"><p className="text-blue-300 text-xs font-medium mb-2">✅ Online Opportunities</p><ul className="flex flex-col gap-1">{websiteScan.opportunities.map((o: string, i: number) => <li key={i} className="text-blue-100 text-sm">• {o}</li>)}</ul></div>}
              </>
            ) : (
          ...
          <div className="bg-white/10 rounded-2xl p-6 mb-6">
            ...
            <p className="text-blue-300 text-sm mb-4">Add your website URL in Settings to enable scanning.</p>
            <button onClick={() => router.push('/settings')} className="bg-white/20 text-white px-4 py-2 rounded-lg text-sm hover:bg-white/30 transition">⚙️ Add Website URL</button>
          </div>
        )}
        ...
        <div className="bg-white/10 rounded-2xl p-6 mb-6">
          ...
          {!hasGrowthAccess ? <UpgradePrompt feature="Local Search Demand" /> : trends ? (
            <>
              <div className="flex items-center gap-4 mb-4">
                <div className={`w-16 h-16 rounded-full flex items-center justify-center text-2xl ${trends.signal === 'high' ? 'bg-green-500' : trends.signal === 'low' ? 'bg-red-500' : 'bg-yellow-500'}`}>{trends.emoji || '📊'}</div>
                <div>
                  <p className={`text-lg font-bold ${trends.signal === 'high' ? 'text-green-400' : trends.signal === 'low' ? 'text-red-400' : 'text-yellow-400'}`}>{trends.signal === 'high' ? '🔥 High Demand' : trends.signal === 'low' ? '📉 Low Demand' : '📊 Normal Demand'}</p>
              ...
              {trends.headlines?.length > 0 && <div className="mt-3"><p className="text-blue-300 text-xs mb-2">Recent headlines:</p><div className="flex flex-col gap-2">{trends.headlines.slice(0, 3).map((h: any, i: number) => <div key={i} className="bg-white/10 rounded-lg p-3"><p className="text-white text-xs font-medium">{h.title}</p><p className="text-blue-300 text-xs mt-1">{h.snippet}</p></div>)}</div></div>}
            </>
          ) : (
            <p className="text-blue-300 text-sm">Click "Load Trends" to see current market intelligence.</p>
          )}
        </div>
        ...
        <div className="bg-white/10 rounded-2xl p-6 mb-6">
          ...
          {!hasGrowthAccess ? <UpgradePrompt feature="Area Demographics" /> : demographics ? (
            <>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                <div className="bg-white/10 rounded-xl p-4"><p className="text-blue-300 text-xs font-medium mb-1">💰 Median Household Income</p><p className="text-white text-2xl font-bold">${parseInt(demographics.medianIncome).toLocaleString()}</p><p className={`text-sm font-medium mt-1 ${demographics.incomeLevel === 'high' ? 'text-green-400' : demographics.incomeLevel === 'upper-middle' ? 'text-blue-300' : demographics.incomeLevel === 'middle' ? 'text-yellow-400' : 'text-red-400'}`}>{demographics.incomeLevel?.charAt(0).toUpperCase() + demographics.incomeLevel?.slice(1)} Income Area</p></div>
                <div className="bg-white/10 rounded-xl p-4"><p className="text-blue-300 text-xs font-medium mb-1">🏘️ Neighbourhood</p><p className="text-blue-100 text-sm">{demographics.neighbourhood}</p></div>
              </div>
            ...
            <p className="text-blue-300 text-sm">Click "Load Demographics" to see income and customer profile data.</p>
          )}
        </div>
          ...
          <div className="bg-white/10 rounded-2xl p-6 mb-6">
            ...
            {performance?.yoy && (
              <div className="grid grid-cols-2 gap-4 mb-4">
                {/* @ts-ignore */}
                <div className="bg-white/10 rounded-xl p-4"><p className="text-blue-300 text-xs font-medium mb-1">This Week vs Last Year</p><p className="text-white text-2xl font-bold">${(performance.yoy as any).thisWeek?.toLocaleString()}</p><div className="flex items-center gap-1 mt-1"><span className={`text-sm font-bold ${(performance.yoy as any).isUp ? 'text-green-400' : 'text-red-400'}`}>{(performance.yoy as any).isUp ? '↑' : '↓'} {Math.abs((performance.yoy as any).changePercent)}%</span><span className="text-blue-300 text-xs">vs ${(performance.yoy as any).lastYear?.toLocaleString()} last year</span></div></div>
                {/* @ts-ignore */}
                {(performance.yoy as any).monthChangePercent !== undefined && (
                  <div className="bg-white/10 rounded-xl p-4"><p className="text-blue-300 text-xs font-medium mb-1">Last 4 Weeks vs Last Year</p><p className="text-white text-2xl font-bold">${(performance.yoy as any).recentTotal?.toLocaleString()}</p><div className="flex items-center gap-1 mt-1"><span className={`text-sm font-bold ${(performance.yoy as any).monthIsUp ? 'text-green-400' : 'text-red-400'}`}>{(performance.yoy as any).monthIsUp ? '↑' : '↓'} {Math.abs((performance.yoy as any).monthChangePercent)}%</span><span className="text-blue-300 text-xs">vs ${(performance.yoy as any).lastYearRecentTotal?.toLocaleString()} last year</span></div></div>
                )}
              </div>
            )}
            ...
            <div className="bg-white/10 rounded-xl p-4"><p className="text-blue-200 text-sm">📅 Your busiest day: <strong className="text-white">{performance.busiestDay}</strong></p></div>
          </div>
        )}
        ...
        {accuracy && (
          <div className="bg-white/10 rounded-2xl p-6 mb-6">
            ...
            <div className="flex items-center gap-6 mb-4">
              ...
              <div className="flex-1">
                <div className="bg-white/10 rounded-full h-4 overflow-hidden"><div className={`h-full rounded-full transition-all ${accuracy.accuracy >= 70 ? 'bg-green-400' : accuracy.accuracy >= 50 ? 'bg-yellow-400' : 'bg-red-400'}`} style={{ width: `${accuracy.accuracy}%` }} /></div>
            ...
            {accuracy.matches?.length > 0 && (
              <div><p className="text-blue-300 text-xs mb-2">Recent predictions:</p><div className="flex flex-col gap-1">{accuracy.matches.map((m: any, i: number) => <div key={i} className="flex justify-between items-center bg-white/10 rounded-lg px-3 py-2"><p className="text-blue-200 text-xs">{m.date}</p><div className="flex items-center gap-2"><span className="text-blue-300 text-xs">Predicted: {m.predicted}</span><span className="text-blue-300 text-xs">→</span><span className="text-blue-300 text-xs">Actual: {m.actual}</span><span>{m.correct ? '✅' : '❌'}</span></div></div>)}</div></div>
            )}
          </div>
        )}
        ...
        <div className="bg-white/10 rounded-2xl p-6 mb-6">
          <div className="flex justify-between items-center">
            <div><h2 className="text-white font-bold text-lg">📂 Import Walk-in Data</h2><p className="text-blue-300 text-sm mt-1">Upload a CSV with your historical walk-in counts</p></div>
            <button onClick={() => setShowImport(!showImport)} className="bg-white/20 text-white px-4 py-2 rounded-lg font-semibold text-sm hover:bg-white/30 transition">{showImport ? 'Hide' : 'Upload CSV'}</button>
          </div>
          {showImport && (
            <div className="mt-4">
              <div className="bg-white/10 rounded-xl p-4 mb-4"><p className="text-blue-200 text-sm font-medium mb-2">📋 Required format:</p><code className="text-green-300 text-xs">date,walkins<br/>2026-05-01,23</code></div>
        ...
        <div className="bg-white/10 rounded-2xl p-6 mb-6">
          <div className="flex justify-between items-center">
            <div><h2 className="text-white font-bold text-lg">💰 Import Previous Year Sales</h2><p className="text-blue-300 text-sm mt-1">Upload last year's daily sales to improve predictions</p></div>
            <button onClick={() => setShowSalesImport(!showSalesImport)} className="bg-white/20 text-white px-4 py-2 rounded-lg font-semibold text-sm hover:bg-white/30 transition">{showSalesImport ? 'Hide' : 'Upload CSV'}</button>
          </div>
          {showSalesImport && (
            <div className="mt-4">
              <div className="bg-white/10 rounded-xl p-4 mb-4"><p className="text-blue-200 text-sm font-medium mb-2">📋 Required format:</p><code className="text-green-300 text-xs">date,revenue<br/>2025-05-01,2450.00</code></div>
              <input type="file" accept=".csv" onChange={async (e) => {
                const file = e.target.files?.[0]; if (!file) return
                setSalesImporting(true); setSalesImportResult('')
                const csvText = await file.text()
                const supabase = createClient()
                const { data: { session } } = await supabase.auth.getSession(); if (!session) return
        ...
        <div className="bg-white/10 rounded-2xl p-6">
          ...
          <div className="flex flex-col gap-2 mb-4 max-h-80 overflow-y-auto">
            {chatMessages.length === 0 && (
              <div className="flex flex-wrap gap-2">
                {['Should I run a sale this Friday?', 'How many staff do I need this weekend?', 'What promotions should I run this week?', 'Is this a good week to launch a new product?'].map(q => (
                  <button key={q} onClick={() => setChatInput(q)} className="bg-white/10 text-blue-200 px-3 py-2 rounded-lg text-xs hover:bg-white/20 transition text-left">{q}</button>
                ))}
              </div>
            )}
            ...
            {chatLoading && <div className="bg-white/10 rounded-xl p-3 mr-8"><p className="text-blue-400 text-xs font-medium mb-1">ShopCast AI</p><p className="text-blue-300 text-sm">Thinking...</p></div>}
          </div>
          <div className="flex gap-2">
            <input type="text" value={chatInput} onChange={e => setChatInput(e.target.value)}
              onKeyDown={async e => {
                if (e.key === 'Enter' && chatInput.trim() && !chatLoading) {
                  ...
                  const data = await res.json()
                  setChatMessages(prev => [...prev, { role: 'assistant', content: data.reply }]); setChatLoading(false)
                }
              }}
            ...
            <button onClick={async () => {
              ...
              const data = await res.json()
              setChatMessages(prev => [...prev, { role: 'assistant', content: data.reply }]); setChatLoading(false)
            }} disabled={chatLoading || !chatInput.trim()} className="bg-white text-blue-900 px-4 py-3 rounded-lg font-semibold text-sm hover:bg-blue-50 transition disabled:opacity-50">Send</button>
          </div>
        </div>
